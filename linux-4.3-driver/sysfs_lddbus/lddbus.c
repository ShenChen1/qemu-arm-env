...
static int ldd_match(struct device *dev, struct device_driver *driver)
{
	...
	return !strncmp(dev_name(dev), driver->name, 5);
}
...
static struct ldd_device sculld_device = {
	.name = "sculld-dev-0",
	.driver = &sculld_driver,
	.dev = {
		.driver_data = &sculld_device.dev,
	},
};
...
static int __init ldd_bus_init(void)
{
	...
	if (ret)
		printk(KERN_NOTICE "Unable to register ldd-bus-dev-0\n");

	register_ldd_driver(&sculld_driver);
	register_ldd_device(&sculld_device);
...
{
	unregister_ldd_device(&sculld_device);
